...
1. **Install the worker** — `uv sync --frozen` into a venv. `.venv/bin/vgi-statsmodels`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run .venv/bin/vgi-statsmodels`.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_STATSMODELS_WORKER` at `uv run .venv/bin/vgi-statsmodels`, warms the
...
VGI_STATSMODELS_WORKER="uv run --python 3.13 .venv/bin/vgi-statsmodels" \
  ci/run-integration.sh
```

Or use the Makefile target `make test-sql`, which installs `haybarn-unittest`
as a uv tool and points the worker at `uv run --python 3.13 .venv/bin/vgi-statsmodels`.
